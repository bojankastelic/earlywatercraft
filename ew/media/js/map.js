    ...
    var PageView = Backbone.View.extend({
        ...
        initialize: function(options) {
            ...
            var showFeaturePopup = function(feature) {
				...
				var userGroups = JSON.parse($('#user-groups').val());
				var userCanEdit = false;   
				_.each(userGroups, function(userGroup) {
				    if ('EDIT_' + typeInfo.entitytypeid == userGroup || 'PUBLISH_' + typeInfo.entitytypeid == userGroup) {
						userCanEdit = true;
					}
 	        	});
 	        	/* Tega ne rabimo, naj se gumb Edit za prijavljene uporabnike vidi, saj je dokument zaklenjen
 	        	if (userCanEdit) {
 	        	    current_status = feature.get('ew_status')
 	        	    // Pri vseh statusih, ki niso draft, zahtevamo tudi pravico PUBLISH
 	        	    if (current_status != 'Draft') {
 	        	        userCanEdit = false
				        _.each(userGroups, function(userGroup) {
				            if ('PUBLISH_' + typeInfo.entitytypeid == userGroup) {
						        userCanEdit = true;
					        }
         	        	});
 	        	    } 
                ...
                if (typeInfo) {
                    ...
                    resourceData.ewStatus = feature.get('ewstatus');
                }
