    ...
    def get_field_map_values(self, resource, template_record, field_map):
        ...
        for mapping in field_map:
            ...
            if 'value_type' in mapping:
                conceptid = mapping[0]['value_type']
            entitytypeid = mapping[0]['entitytypeid']
            alternates = False
            ...
            if 'alternate_entitytypeid' in mapping:
                alternates = True
                alternate_entitytypeid = mapping[0]['alternate_entitytypeid']
                alternate_values = []
            ...
            if entitytypeid.endswith('E55') != True:
                for entity in child_entities:
                    ...
                    if entitytypeid == entity['entitytypeid'] and conceptid == '':
                        template_record[mapping[0]['field_name']].append(entity['value'])
                ...
                for domain in domains:
                    if entitytypeid == domain['entitytypeid']:
                        template_record[mapping[0]['field_name']].append(domain['label'])
                    if alternate_entitytypeid == domain['entitytypeid']:
                        alternate_values.append(entity['value'])

            if alternates == True and len(template_record[mapping[0]['field_name']]) == 0:
                if len(alternate_values) > 0:
                    template_record[mapping[0]['field_name']] = alternate_values
